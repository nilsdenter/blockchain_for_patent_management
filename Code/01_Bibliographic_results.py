...
countries = list(country_contributions)
contributions = []
for country in list(country_contributions):
    contributions.append(country_contributions[country])

df = pd.DataFrame(data=zip(countries, contributions), columns =["Countries", "No. of observations"])
df.sort_values(["No. of observations", "Countries"], inplace=True, ascending=[False, True])
...
df.to_excel("top_countries_by_observations.xlsx", index=False)
...
plt.tight_layout()
plt.savefig("Figure 4.png", dpi=1600)
plt.savefig("Figure 4.svg", format='svg', dpi=1600)
plt.plot()